            ...
            <div>
                <nav className={`flex items-center justify-between px-6 p-4 fixed top-0 right-0 left-0 transition-transform duration-300 bg-black z-50 ${MostrarHeader ? "translate-y-0" : "-translate-y-full"}`}>
                    <div className="group flex hover:bg-black" onMouseEnter={abrir} onMouseLeave={abrir}>
                        <p className="cursor-default flex gap-2 text-sm lg:text-base">¡Hola! {nombre?.split(' ')[0] || nombre?.substring(0, 25)}
                            <div className="w-5 h-5 flex mt-2">
                                <span className={`block h-0.5 w-2 bg-current transform transition-all duration-300 origin-center ${desplegable ? 'rotate-[135deg] translate-y-1' : 'rotate-[45deg] translate-y-1'}`}></span>
                                <span className={`block h-0.5 w-2 bg-current transform transition-all duration-300 origin-center -ml-1 ${desplegable ? '-rotate-[135deg] translate-y-1' : '-rotate-[45deg] translate-y-1'}`}></span>
                            </div>
                        </p>
            ...
            <div className="p-5 h-64 sm:h-80 md:h-96 bg-cover bg-center bg-no-repeat text-white bg-[url('/src/assets/image/Lettering.jpeg')]">
                ...
                <div className={`fixed z-50 bg-Suavizado w-full h-dvh top-0 left-0 transition-all duration-500 ${!isOpenCart && "invisible"}`}>
                    <div className={`bg-black h-dvh ml-auto fixed top-0 left-0 right-0 transition-transform duration-300 ease-in-out sm:carrito p-8 ${isOpenCart ? "translate-x-0" : "translate-x-full"}`}>
                        ...
                        <label onClick={() => setIsOpenCart(false)} htmlFor="" className="absolute right-10 cursor-pointer text-xl font-bold" onMouseEnter={abrir} onMouseLeave={abrir}>
                            <p className={`transform transition-all duration-300 origin-center ${desplegable ? '-rotate-[360deg] scale-150' : ''}`}>x</p>
                        </label>
