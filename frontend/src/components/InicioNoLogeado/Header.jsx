                ...
                <div className={`fixed z-50 bg-Suavizado ml-auto w-full h-dvh top-0 left-0 transition-all duration-300 ${!isOpenAccount && "invisible"}`}>
                    <form onSubmit={click}>
                        <div className={`bg-black z-50 h-dvh fixed left-0 right-0 top-0 transition-transform duration-300 ease-in-out sm:cuenta p-8 ${isOpenAccount ? "translate-x-0" : "-translate-x-full"}`}>
                            ...
                            <div className="mt-8">
                                <label>Correo Electronico <span className="text-RosadoOcobo">*</span> <br /><input id="correo" name="correo" value={correo} onChange={handleChange} className="rounded-md h-9 w-full text-black outline-none p-2" type="text" /></label> <br /> <br />
                                <label className="relative">Contraseña <span className="text-RosadoOcobo">*</span>  <br /> <EyeToggle visible={verContrasena} onClick={verOcultarContrasena} className="absolute top-[1.9rem] left-[17rem] text-black" /> <input id="contrasena" name="contrasena" value={contrasena} onChange={handleChange} className="rounded-md h-9 w-full text-black outline-none p-2" type={verContrasena ? "text" : "password"} /></label>
